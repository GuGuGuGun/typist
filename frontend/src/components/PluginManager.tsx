...
import { api } from '../api';
import { X, Blocks, Trash2, Shield, DownloadCloud, AlertCircle } from 'lucide-react';
...
export const PluginManager: React.FC = () => {
  const isOpen = useStore(state => state.isPluginsOpen);
  const toggleOpen = useStore(state => state.togglePlugins);
  ...
  const handleToggle = async (id: string, currentStatus: string) => {
    try {
      if (currentStatus === 'active') {
        ...
        // MVP: If it's a JS plugin, we need to load its script and run it
        const plugin = plugins.find(p => p.manifest.id === id);
        if (plugin?.manifest.entry) {
           const script = document.createElement('script');
           // In Tauri, loading local scripts might require `tauri://` or `http://localhost`.
           // Given this MVP, we simulate fetching via backend or load from standard DOM.
           // `entry` could be a URL or a file path.
  ...
  const handleLoadCustom = async () => {
    try {
      ...
      if (manifestPath) {
        setStatusMsg('Loading plugin...');
        // Assume backend can handle reading it, though we'd need to parse it or let backend do it.
        // Actually the backend `register_plugin_cmd` takes a RegisterPluginRequest.
        // Since we are mocking the MVP registration, we just prompt an error if not implemented directly.
    ...
    <div className="plugin-modal-overlay">
      <div className="plugin-modal">
        <div className="plugin-modal-header">
          <h2 className="plugin-modal-title">
            <Blocks size={20} className="text-accent" /> Plugin Marketplace
          </h2>
          <div className="plugin-modal-close" onClick={toggleOpen}>
            <X size={18} />
          </div>
        </div>
        
        <div className="plugin-modal-toolbar">
          <span className="plugin-toolbar-text">
            Enhance your editor with third-party extensions.
          </span>
          <button onClick={handleLoadCustom} className="plugin-load-btn">
