...
- [x] 实现 Plugin Manager
  ...
  - [x] 前端：Load Plugin 支持读取 manifest 并自动完成注册、激活与脚本注入
- [x] 实现 UI 插槽机制
...
- [x] 将"字数统计"和"大纲视图"重构为内置插件，验证插件架构
  - [x] 前后端联调：抽离为标竿官方插件
- [x] 编写插件构建文档
  - [x] 文档：`docs/PLUGIN_BUILD.md`（manifest 规范、构建产物要求、加载与调试流程）
...
- 已补齐自动保存调度链路：按设置项 `autosave_enabled` 与 `autosave_interval_secs` 周期执行脏标签自动保存。
